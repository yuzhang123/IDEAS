...
///////////////////////////////////////////////////
public: 
	void inferStructure(MYDATA &mydata, TENSORPARA &tpara, char const *outname, bool outqc);
...
protected:
	...
	void _forwardSum(MYDATA &mydata, TENSORPARA &tpara, int id, int ploidity, vector<vector<int> > const &ss, vector<vector<int> > const &mlist, vector<int> const &dbreaks, double *Vh2, double *P); 
	double _backwardSample(MYDATA &mydata, TENSORPARA &tpara, int id, int ploidity, vector<vector<int> > const &ss, vector<vector<int> > const &mlist, vector<int> const &dbreaks, double *Vh2, double *P);
	...
	virtual double _getLp(int id, vector<int> const &breaks, int bid, bool flip, int ploidity, TENSORPARA const &tpara, float const *dp, float const *dp2, int const *pp, int const *pp2, vector<vector<vector<float> > >::const_iterator ipm, int const *nhp, int const *aszp)=0;
	virtual double _imputeOne(int id, vector<int> const &breaks, int bst, int bed, int flip, int ploidity, TENSORPARA const &tpara, float *dp, float *dp2, int const *pp, int const *pp2, vector<vector<vector<float> > >::const_iterator ipm, int const *nhp, int const *aszp)=0;
	virtual void _updatePrior(vector<bool> const &init, bool oneround = false)=0;

private:
	int _shrinkClasses(int indN, int L, float *z);
	void _removeInd(MYDATA const &mydata, TENSORPARA &tpara, int id, int wb = 1, int st = -1, int ed = -1, bool clean = false);
        void _addInd(MYDATA const &mydata, TENSORPARA &tpara, int id, int wb = 1, int st = -1, int ed = -1);
        int _updateRecomb(TENSORPARA &tpara, int trueindN, int L);
	...
	void _greedyMatch(int *trans, int maxK, int *perm);
	double _imputeData(MYDATA const &mydata, TENSORPARA const &tpara, int id, vector<int> const &dbreaks);
